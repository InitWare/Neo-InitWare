 ...
 *
 * This module implements loading of legacy systemd unit files for InitKit.
...
const edgeTypes = Scheduler.edgeTypes;
...
function unitType(name) {
	...
	return idx != -1 ? name.slice(idx + 1) : "service";
}
...
export function loadSystemdUnit(name) {
	...
	let edges_to = {};
	/**
	 * The object type, e.g. target, service, [...].
	 * @type {String}
	 */
	let objtype;

	({ aliases, obj } = readSystemdUnit(name));

	if (obj == null)
		return -1;

	objtype = unitType(aliases[0]);
	...
	for (const depType of ["Before", "After", "Requires", "Wants",
		...
		depsForType.forEach((el) => {
			...
			if (dependencyToEdgeTypes[depType] != 0)
			{
				...
				if (targdeps)
					edges_from[el] |= edgeTypes.kAfter;
			}
				...
				dependencyToEdgeTypes[complement] != 0)
			{
				...
				if (targdeps)
					edges_to[el] |= edgeTypes.kBefore;
			}
		});

	}

	Scheduler.scheduler.objectLoad(aliases, edges_from, edges_to);

	return 0;
}

globalThis.loadObject = loadSystemdUnit;
