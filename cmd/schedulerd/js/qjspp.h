...
struct js_traits<Int,
    std::enable_if_t<(std::is_integral_v<Int> || std::is_enum_v<Int>)&&sizeof(
			 Int) <= sizeof(int64_t)>> {
...
template <typename... Ts> struct js_traits<std::variant<Ts...>> {
	 ...
	 * etc.) */
	template <template <typename R> typename Trait, typename U,
	    typename... Us>
	...
	static std::optional<std::variant<Ts...>> unwrapObj(JSContext *ctx,
	    JSValueConst v, JSClassID class_id)
	{
		...
		// try to unwrap embedded variant (variant<variant<...>>), might
		// be slow
		if constexpr (is_variant<U>::value) {
			if (auto opt = js_traits<std::optional<U>>::unwrap(ctx,
				v))
				return *opt;
		}
		...
		if constexpr (is_vector<U>::value) {
			if (JS_IsArray(ctx, v) == 1) {
				auto firstElement = JS_GetPropertyUint32(ctx, v,
				    0);
				bool ok = isCompatible<
				    std::decay_t<typename U::value_type>>(ctx,
				    firstElement);
				JS_FreeValue(ctx, firstElement);
				if (ok) {
					return U { js_traits<U>::unwrap(ctx,
					    v) };
				}
			}
		}
		...
		if constexpr (is_pair<U>::value) {
			if (JS_IsArray(ctx, v) == 1) {
				...
				auto firstElement = JS_GetPropertyUint32(ctx, v,
				    0);
				auto secondElement = JS_GetPropertyUint32(ctx,
				    v, 1);
				bool ok =
				    isCompatible<
					std::decay_t<typename U::first_type>>(
					ctx, firstElement) &&
				    isCompatible<
	...
	static std::variant<Ts...> unwrapPriority(JSContext *ctx,
	    JSValueConst v)
	{
	...
	static std::variant<Ts...> unwrap(JSContext *ctx, JSValueConst v)
	{
		...
		case JS_TAG_INT:
			[[fallthrough]];
		case JS_TAG_BIG_INT:
			return unwrapPriority<std::is_integral,
		...
		default: // more than JS_TAG_FLOAT64 (nan boxing)
			return unwrapPriority<is_double,
			    std::is_floating_point>(ctx, v);
		}
...
template <typename T>
T
unwrap_free(JSContext *ctx, JSValue val)
{
...
Tuple
unwrap_args_impl(JSContext *ctx, JSValueConst *argv, std::index_sequence<I...>)
{
	return Tuple {
		js_traits<std::decay_t<std::tuple_element_t<I, Tuple>>>::unwrap(
		    ctx, argv[I])...
	};
}
...
unwrap_args(JSContext *ctx, JSValueConst *argv)
{
	return unwrap_args_impl<std::tuple<std::decay_t<Args>...>>(ctx, argv,
...
JSValue
wrap_call(JSContext *ctx, Callable &&f, JSValueConst *argv) noexcept
{
	try {
...
JSValue
wrap_this_call(JSContext *ctx, Callable &&f, JSValueConst this_value,
    JSValueConst *argv) noexcept
{
	try {
		if constexpr (std::is_same_v<R, void>) {
			std::apply(std::forward<Callable>(f),
			    std::tuple_cat(unwrap_args<FirstArg>(ctx,
					       &this_value),
				unwrap_args<Args...>(ctx, argv)));
			return JS_NULL;
		} else {
...
template <class Tuple, std::size_t... I>
void
wrap_args_impl(JSContext *ctx, JSValue *argv, Tuple tuple,
...
template <typename... Args>
void
wrap_args(JSContext *ctx, JSValue *argv, Args &&...args)
{
		...
		return JS_NewCFunction(
			...
			JSValueConst *argv) noexcept -> JSValue {
			    if constexpr (PassThis)
					...
					ctx, F, this_value, argv);
			    else
				    return detail::wrap_call<R, Args...>(ctx, F,
					argv);
		    },
		    fw.name, sizeof...(Args));
	}
...
struct js_traits<fwrapper<F, PassThis>> {
	static JSValue wrap(JSContext *ctx, fwrapper<F, PassThis> fw) noexcept
	{
		return JS_NewCFunction(
		    ...
		    [](JSContext *ctx, JSValueConst this_value, int argc,
			JSValueConst *argv) noexcept -> JSValue {
			    return detail::wrap_this_call<R, std::shared_ptr<T>,
				Args...>(ctx, F, this_value, argv);
		    },
		    fw.name, sizeof...(Args));
	}
};
...
struct js_traits<ctor_wrapper<T, Args...>> {
	static JSValue wrap(JSContext *ctx,
	    ctor_wrapper<T, Args...> cw) noexcept
	{
		return JS_NewCFunction2(
			...
			JSValueConst *argv) noexcept -> JSValue {
			    ...
			    {
#if defined(__cpp_rtti)
				    // automatically register class on first use
				    // (no prototype)
				    js_traits<
					std::shared_ptr<T>>::register_class(ctx,
					typeid(T).name());
#else
				    JS_ThrowTypeError(ctx,
			    ...
			    auto proto = JS_GetPropertyStr(ctx, this_value,
				"prototype");
			    if (JS_IsException(proto))
				    return proto;
			    auto jsobj = JS_NewObjectProtoClass(ctx, proto,
			    ...
			    // Args...>(ctx, std::make_shared<T, Args...>,
			    // argv);
		    },
...
template <class T> struct js_traits<std::shared_ptr<T>> {
	...
	{
		auto ptr = reinterpret_cast<std::shared_ptr<T> *>(
		    JS_GetOpaque2(ctx, v, QJSClassId));
		if (!ptr)
			throw exception {};
		return *ptr;
	}
...
template <class T> struct js_traits<T *, std::enable_if_t<std::is_class_v<T>>> {
	static JSValue wrap(JSContext *ctx, T *ptr)
	{
		...
		auto jsobj = JS_NewObjectClass(ctx,
		    js_traits<std::shared_ptr<T>>::QJSClassId);
		if (JS_IsException(jsobj))
			return jsobj;
 ...
 */
struct function {
	JSValue (*invoker)(function *self, JSContext *ctx,
	    JSValueConst this_value, int argc, JSValueConst *argv) = nullptr;
	...
	{
		auto fptr = reinterpret_cast<function *>(
		    js_malloc_rt(rt, sizeof(function) + sizeof(Functor)));
		if (!fptr)
			throw std::bad_alloc {};
		new (fptr) function;
		...
		if constexpr (!std::is_trivially_destructible_v<Functor>) {
			fptr->destroyer = [](function *fptr) {
				auto functorptr = reinterpret_cast<Functor *>(
				    fptr->functor);
				functorptr->~Functor();
			};
		}
...
template <> struct js_traits<detail::function> {
	...
	static void register_class(JSContext *ctx, const char *name)
	{
		...
		JSClassDef def { name,
			...
			[](JSRuntime *rt, JSValue obj) noexcept {
				auto fptr =
				    reinterpret_cast<detail::function *>(
					JS_GetOpaque(obj, QJSClassId));
				assert(fptr);
				if (fptr->destroyer)
					fptr->destroyer(fptr);
			...
			[](JSContext *ctx, JSValueConst func_obj,
			    ...
			    int flags) -> JSValue {
				auto ptr = reinterpret_cast<detail::function *>(
				    JS_GetOpaque2(ctx, func_obj, QJSClassId));
				if (!ptr)
					return JS_EXCEPTION;
				return ptr->invoker(ptr, ctx, this_val, argc,
				    argv);
			},
			nullptr };
		int e = JS_NewClass(rt, QJSClassId, &def);
...
template <typename Key> struct property_proxy {
	...
	operator Value()
	    const; // defined later due to Value being incomplete type

	template <typename Value> property_proxy &operator=(Value value)
	{
...
class Value {
	...
	{
		(*this)[name] = js_traits<decltype(
		    std::function { std::forward<Function>(f) })>::wrap(ctx,
	...
	std::string toJSON(const Value &replacer = Value { nullptr,
			       JS_UNDEFINED },
	    const Value &space = Value { nullptr, JS_UNDEFINED })
	{
		assert(ctx);
...
class Context {
	...
	class Module {
		    ...
		    , name(name)
		{
			m = JS_NewCModule(ctx, name,
			    [](JSContext *ctx, JSModuleDef *m) noexcept {
				    ...
				    auto it =
					std::find_if(context.modules.begin(),
					    context.modules.end(),
		...
		/** Add free function F.
		 * Example:
		 * module.function<static_cast<double
		 * (*)(double)>(&::sin)>("sin");
		 */
		 ...
		 * Example: module.function("sin", [](double x) { return
		 * ::sin(x); });
		 */
		template <typename F> Module &function(const char *name, F &&f)
		{
			return add(name,
			    js_traits<decltype(std::function { std::forward<F>(
				f) })>::wrap(ctx, std::forward<F>(f)));
		}
		...
		template <class T> class class_registrar {
			...
			explicit class_registrar(const char *name,
			    qjs::Context::Module &module, qjs::Context &context)
			    : name(name)
			...
			class_registrar &property(const char *name)
			{
				...
				if constexpr (FSet == nullptr)
					...
					prototype.add_getter_setter<FGet, FSet>(
					    name);
				return *this;
			}
			 ...
			 * @param name constructor name (if not specified class
			 * name will be used)
			 */
			template <typename... Args>
			class_registrar &constructor(const char *name = nullptr)
			{
				if (!name)
					name = this->name;
				Value ctor = context.newValue(
				    qjs::ctor_wrapper<T, Args...> { name });
				JS_SetConstructor(context.ctx, ctor.v,
				    prototype.v);
			...
			"base class is not registered"); auto base_proto =
			JS_GetClassProto(context.ctx,
			js_traits<std::shared_ptr<B>>::QJSClassId); int err =
		...
		{
			return class_registrar<T> { name, *this,
				qjs::Context::get(ctx) };
		}
	};
	...
	void init()
	{
		...
		js_traits<detail::function>::register_class(ctx,
		    "C++ function");
	}
	...
	/** returns current exception associated with context, and resets it.
	    ...
	    unsigned eval_flags = 0)
	{
		assert(buffer.data()[buffer.size()] == '\0' &&
		    "eval buffer is not null-terminated"); // JS_Eval
							   // requirement
		JSValue v = JS_Eval(ctx, buffer.data(), buffer.size(), filename,
		    eval_flags);
		return Value { ctx, v };
	}
	...
	Value evalFile(const char *filename, unsigned eval_flags = 0)
	{
		...
		if (!buf)
			throw std::runtime_error {
				std::string { "evalFile: can't read file: " } +
				filename
			};
		return eval({ reinterpret_cast<char *>(buf.get()), buf_len },
		    filename, eval_flags);
	}
	    ...
	    const char *filename = "<fromJSON>")
	{
		assert(buffer.data()[buffer.size()] == '\0' &&
		    "fromJSON buffer is not null-terminated"); // JS_ParseJSON
							       // requirement
		JSValue v = JS_ParseJSON(ctx, buffer.data(), buffer.size(),
		    filename);
		return Value { ctx, v };
	}
...
struct js_traits<std::function<R(Args...)>> {
	static auto unwrap(JSContext *ctx, JSValueConst fun_obj)
	{
		...
		if constexpr (argc == 0) {
			return [jsfun_obj = Value { ctx,
				    JS_DupValue(ctx, fun_obj) }]() -> R {
				JSValue result = JS_Call(jsfun_obj.ctx,
				    jsfun_obj.v, JS_UNDEFINED, 0, nullptr);
				if (JS_IsException(result)) {
					JS_FreeValue(jsfun_obj.ctx, result);
					throw exception {};
				}
				return detail::unwrap_free<R>(jsfun_obj.ctx,
				    result);
			};
		} else {
			return [jsfun_obj = Value { ctx,
	...
	static JSValue wrap(JSContext *ctx, Functor &&functor)
	{
		...
		auto obj = JS_NewObjectClass(ctx,
		    js_traits<function>::QJSClassId);
...
template <class T> struct js_traits<std::vector<T>> {
	...
	static std::vector<T> unwrap(JSContext *ctx, JSValueConst jsarr)
	{
		...
		if (e == 0)
			JS_ThrowTypeError(ctx,
			    "js_traits<std::vector<T>>::unwrap expects array");
		if (e <= 0)
			throw exception {};
...
template <typename T> struct js_traits<std::optional<T>> {
	...
	static auto unwrap(JSContext *ctx, JSValueConst v) noexcept
	    -> std::optional<decltype(js_traits<std::decay_t<T>>::unwrap(ctx,
		v))>
	{
		try {
			if (JS_IsNull(v))
